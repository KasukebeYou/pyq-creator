...
(function () {
  ...
  ready(() => {
    ...
    (function enableFabDrag() {
      ...
      panel.innerHTML = `
        <div class="sp-header">
          ...
          <div style="font-size:12px; color:#999">v0.1.1 (Fixed)</div>
        </div>
     ...
     function showChatConfig() {
        ...
        function renderMessagesForDebug(messages) {
            ...
            if (!debugArea) return;
            const messageContent = messages.map((text, i) => `[${i}] ${text}`).join('\n');
            debugLog('聊天记录预处理结果:', messageContent);
        }
    ...
    // **FIX #2 & #3: 完整的、已修复的 `showGenPanel` 函数**
    function showGenPanel() {
        ...
        const PROMPTS_KEY = 'friendCircleUserPrompts';
        const LAST_GEN_OUTPUT_KEY = 'friendCircleLastGenOutput'; // 用于存储输出的键

        // 恢复上次的生成内容
        const savedOutput = localStorage.getItem(LAST_GEN_OUTPUT_KEY);
        if (savedOutput) {
            outputContainer.textContent = savedOutput;
        }
        ...
        async function getLastMessages() {
            // (此函数与聊天配置中的版本相同，此处为简化省略，实际代码中已包含)
            try {
                const ctx = SillyTavern.getContext();
                if (!ctx || !Array.isArray(ctx.chat)) return [];
                const count = parseInt(localStorage.getItem('friendCircleChatCount') || 10, 10);
        ...
        async function generateFriendCircle(selectedChat = [], selectedWorldbooks = []) {
            ...
            try {
                ...
                localStorage.setItem(LAST_GEN_OUTPUT_KEY, output); // 保存生成结果
                ...
                localStorage.setItem(LAST_GEN_OUTPUT_KEY, errorMsg); // 保存错误信息
                ...
                autoObserver = new MutationObserver(() => {
                    ...
                    if (ctx.chat.length > lastMessageCount) {
                        ...
                        if (newMsg && !newMsg.is_user && newMsg.mes) {
                            ...
                            getLastMessages().then(cutted => { generateFriendCircle(cutted, []); }).catch(err => { console.error('自动模式获取最新消息失败:', err); });
                        }
                    }
                });
        ...
        if (savedAutoMode === '1') { toggleAutoMode(true); }
        ...
        document.getElementById('sp-gen-now').addEventListener('click', async () => {
            try {
                ...
                if (localStorage.getItem(WORLDBOOK_TOGGLE_KEY) === 'true') {
                    const ctx = SillyTavern.getContext();
                    const allWorldInfoEntries = window.SillyTavern.worldInfo;

                    if (ctx && Array.isArray(allWorldInfoEntries)) {
                        ...
                        selectedWorldbooks = allWorldInfoEntries
                            .filter(entry =>
                                activeBookFiles.includes(entry.book) &&
                                entry.enabled &&
                        ...
                        debugLog(`已从启用的世界书 (${activeBookFiles.join(', ') || '无'}) 中读取 ${selectedWorldbooks.length} 条条目。`);
                         ...
                         debugLog('无法获取世界书数据 (window.SillyTavern.worldInfo 不可用)');
                    }
                } else {

  ...
  ready(() => {
    try {
      ...
      function debugLog(...args) {
          const dbg = document.getElementById('sp-debug');
          if (!dbg) return;
          const newText = args.map(arg => typeof arg === 'object' ? JSON.stringify(arg, null, 2) : String(arg)).join(' ');
      ...
      function showApiConfig() {
          ...
          function populateModelSelect(models) {
              ...
              uniq.forEach(m => { const opt = document.createElement("option"); opt.value = m; opt.textContent = m; modelSelect.appendChild(opt); });
              if (savedModel) {
                  let existing = Array.from(modelSelect.options).find(o => o.value === savedModel);
                  if (existing) { existing.textContent = savedModel + "（已保存）"; modelSelect.value = savedModel; }
                  else { const opt = document.createElement("option"); opt.value = savedModel; opt.textContent = savedModel + "（已保存）"; modelSelect.insertBefore(opt, modelSelect.firstChild); modelSelect.value = savedModel; }
              } else if (modelSelect.options.length > 0) { modelSelect.selectedIndex = 0; }
          }
          ...
          document.getElementById("api-save-btn").addEventListener("click", () => {
              const url = document.getElementById("api-url-input").value; const key = document.getElementById("api-key-input").value; const model = modelSelect.value;
              if (!url || !key || !model) return alert("请完整填写API信息");
              localStorage.setItem("independentApiUrl", url); localStorage.setItem("independentApiKey", key); localStorage.setItem("independentApiModel", model);
              Array.from(modelSelect.options).forEach(o => { if (o.value === model) o.textContent = model + "（已保存）"; else if (o.textContent.endsWith("（已保存）")) o.textContent = o.value; });
              document.getElementById("api-status").textContent = "已保存"; debugLog("保存API配置", { url, model });
          });
          document.getElementById("api-test-btn").addEventListener("click", async () => {
              const urlRaw = document.getElementById("api-url-input").value || localStorage.getItem("independentApiUrl"); const key = document.getElementById("api-key-input").value || localStorage.getItem("independentApiKey"); const model = modelSelect.value || localStorage.getItem("independentApiModel");
              if (!urlRaw || !key || !model) return alert("请完整填写API信息");
              const baseUrl = urlRaw.replace(/\/$/, ""); document.getElementById("api-status").textContent = "正在向模型发送 ping ..."; debugLog("测试连接开始", { baseUrl, model });
              try {
                  const res = await fetch(`${baseUrl}/v1/chat/completions`, { method: "POST", headers: { "Authorization": `Bearer ${key}`, "Content-Type": "application/json" }, body: JSON.stringify({ model, messages: [{ role: "user", content: "ping" }], max_tokens: 100 }) });
                  if (!res.ok) throw new Error(`chat/completions 返回 ${res.status}`); const data = await res.json();
                  document.getElementById("api-status").textContent = `模型 ${model} 可用（ping 成功）`; debugLog("ping 成功", data);
              } catch (e) { document.getElementById("api-status").textContent = "连接失败: " + (e.message || e); debugLog("ping 失败", e.message || e); }
          });
          async function fetchAndPopulateModels(force = false) {
              const url = document.getElementById("api-url-input").value || localStorage.getItem("independentApiUrl"); const key = document.getElementById("api-key-input").value || localStorage.getItem("independentApiKey");
              ...
              try {
                  const res = await fetch(`${url.replace(/\/$/, "")}/v1/models`, { headers: { Authorization: `Bearer ${key}` } }); const data = await res.json(); debugLog("拉取模型原始返回", data);
                  const ids = parseModelIdsFromResponse(data); if (ids.length === 0) throw new Error("未解析到模型");
                  localStorage.setItem("independentApiModels", JSON.stringify(ids)); localStorage.setItem("independentApiModelsFetchedAt", String(Date.now()));
                  populateModelSelect(ids); document.getElementById("api-status").textContent = `已拉取 ${ids.length} 个模型`;
          ...
          function parseModelIdsFromResponse(data) {
              if (!data) return []; if (Array.isArray(data.data)) return data.data.map(m => m.id || m.model || m.name).filter(Boolean);
              if (Array.isArray(data.models)) return data.models.map(m => m.id || m.model || m.name).filter(Boolean); if (Array.isArray(data)) return data.map(m => m.id || m.model || m.name).filter(Boolean);
              if (data.model) return [data.model]; if (data.id) return [data.id]; return [];
          }
          document.getElementById("api-refresh-models-btn").addEventListener("click", async () => { debugLog("手动刷新模型"); await fetchAndPopulateModels(true); });
      ...
      function showPromptConfig() {
          content.innerHTML = `
              <div>
                  <textarea rows="3" id="sp-prompt-text" placeholder="输入提示词"></textarea><br>
                  <div id="sp-prompt-list"></div>
                  <input type="text" id="sp-prompt-search" placeholder="按标签搜索" style="width: 70%; margin-top: 8px;">
                  <button id="sp-prompt-search-btn" style="margin-left: 8px;">搜索</button>
                  <button id="save-prompts-btn" style="margin-top: 12px; width: 100%;">保存提示词</button>
              </div>
          `;
          const PROMPTS_KEY = 'friendCircleUserPrompts'; let friendCirclePrompts = []; let promptTagFilter = "";
          function loadUserPrompts() { friendCirclePrompts = JSON.parse(localStorage.getItem(PROMPTS_KEY) || '[]'); }
          function renderPromptList() {
              const container = document.getElementById('sp-prompt-list'); container.innerHTML = '';
              friendCirclePrompts.forEach((p, idx) => {
                  if (promptTagFilter && !p.tags.some(tag => tag.toLowerCase().includes(promptTagFilter))) return;
                  const div = document.createElement('div'); div.style.marginBottom = '8px'; div.style.borderBottom = '1px solid #eee'; div.style.paddingBottom = '6px';
                  const row = document.createElement('div'); row.style.display = 'flex'; row.style.alignItems = 'center';
                  const checkbox = document.createElement('input'); checkbox.type = 'checkbox'; checkbox.checked = p.enabled || false; checkbox.style.marginRight = '8px';
                  ...
                  const span = document.createElement('span'); span.textContent = p.text; span.style.flex = '1'; span.style.overflow = 'hidden'; span.style.textOverflow = 'ellipsis'; span.style.whiteSpace = 'nowrap';
                  const editBtn = document.createElement('button'); editBtn.textContent = '✏️'; editBtn.style.marginLeft = '8px';
                  editBtn.addEventListener('click', () => { const input = document.createElement('input'); input.type = 'text'; input.value = p.text; input.style.flex = '1'; row.replaceChild(input, span); input.addEventListener('blur', () => { const newText = input.value.trim(); if (newText) { friendCirclePrompts[idx].text = newText; localStorage.setItem(PROMPTS_KEY, JSON.stringify(friendCirclePrompts)); } renderPromptList(); }); input.focus(); });
                  const tagBtn = document.createElement('button'); tagBtn.textContent = '🏷️'; tagBtn.style.marginLeft = '8px';
                  tagBtn.addEventListener('click', () => { const newTag = prompt('输入标签:'); if (newTag) { if (!Array.isArray(friendCirclePrompts[idx].tags)) friendCirclePrompts[idx].tags = []; friendCirclePrompts[idx].tags.push(newTag); localStorage.setItem(PROMPTS_KEY, JSON.stringify(friendCirclePrompts)); renderPromptList(); } });
                  ...
                  row.appendChild(checkbox); row.appendChild(span); row.appendChild(editBtn); row.appendChild(tagBtn); row.appendChild(delBtn); div.appendChild(row);
                  if (p.tags && p.tags.length > 0) { const tagsRow = document.createElement('div'); tagsRow.style.marginLeft = '20px'; tagsRow.style.marginTop = '6px'; p.tags.forEach((t, tIdx) => { const tagEl = document.createElement('span'); tagEl.textContent = t; tagEl.style.cssText = 'display:inline-block; padding:4px 8px; margin:0 6px 6px 0; font-size:12px; border-radius:10px; background:#e0e0e0; cursor:pointer;'; tagEl.title = '点击删除标签'; tagEl.addEventListener('click', () => { friendCirclePrompts[idx].tags.splice(tIdx, 1); localStorage.setItem(PROMPTS_KEY, JSON.stringify(friendCirclePrompts)); renderPromptList(); }); tagsRow.appendChild(tagEl); }); div.appendChild(tagsRow); }
          ...
          document.getElementById('sp-prompt-text').addEventListener('blur', () => { const promptText = document.getElementById('sp-prompt-text').value.trim(); if (promptText) { friendCirclePrompts.push({ text: promptText, enabled: true, tags: [] }); localStorage.setItem(PROMPTS_KEY, JSON.stringify(friendCirclePrompts)); document.getElementById('sp-prompt-text').value = ''; renderPromptList(); } });
      ...
      function showChatConfig() {
          content.innerHTML = `
          <div>
              <div style="display:flex; align-items:center; margin-bottom:12px;"><span style="margin-right:10px;">读取聊天条数: </span><input type="range" id="sp-chat-slider" min="0" max="20" value="10" style="flex:1;"><span id="sp-chat-slider-value" style="margin-left:4px;">10</span></div>
          ...
          const savedCount = localStorage.getItem('friendCircleChatCount'); if (savedCount) { sliderInput.value = savedCount; sliderValue.textContent = savedCount; }
          ...
          function setupList(containerId, inputId, buttonId, storageKey, type) {
              ...
              const loadList = () => {
                  const list = JSON.parse(localStorage.getItem(storageKey) || '[]'); container.innerHTML = '';
                  list.forEach((item, idx) => {
                      ...
                      div.appendChild(checkbox); div.appendChild(text); div.appendChild(editBtn); div.appendChild(delBtn); container.appendChild(div);
                  });
              };
              button.addEventListener('click', () => { const val = input.value.trim(); if (!val) return; const list = JSON.parse(localStorage.getItem(storageKey) || '[]'); list.push({ pattern: val, enabled: true }); localStorage.setItem(storageKey, JSON.stringify(list)); input.value = ''; loadList(); });
              loadList();
          }
          ...
          async function getLastMessages() {
              try {
                  const ctx = SillyTavern.getContext(); if (!ctx || !Array.isArray(ctx.chat)) return [];
                  const count = parseInt(localStorage.getItem('friendCircleChatCount') || 10, 10); if (count === 0) return [];
                  ...
                  const processedMessages = lastMessages.map(msg => { let text = msg.mes || msg.original_mes || ""; if (tagFilterList.length > 0) { const extracts = []; tagFilterList.forEach(tagPattern => { const tagName = tagPattern.replace(/[<>/\s]/g, ''); if (!tagName) return; const regex = new RegExp(`<${tagName}>([\\s\\S]*?)<\\/${tagName}>`, 'g'); let match; while ((match = regex.exec(text)) !== null) { extracts.push(match[1].trim()); } }); text = extracts.join('\n'); } regexTrimList.forEach(regex => { text = text.replace(regex, ''); }); return text.trim(); }).filter(Boolean);
                  localStorage.setItem('cuttedLastMessages', JSON.stringify(processedMessages));
                  const messageContent = processedMessages.map((text, i) => `[${i}] ${text}`).join('\n'); debugLog('聊天记录预处理结果:\n' + messageContent); return processedMessages;
              } catch (e) { console.error('getLastMessages 出错', e); return []; }
          }
          function fetchAndCountMessages() { getLastMessages(); }
          fetchAndCountMessages(); debugLog('进入 聊天配置面板');
      }

      // 世界书面板
      function showWorldbookPanel() {
          content.innerHTML = `
              <h4>世界书条目选择器</h4>
              <div style="margin-bottom: 10px; display: flex; align-items: center;">
                  <input type="checkbox" id="sp-gen-toggle-worldbook" style="margin-right: 6px;">
                  <label for="sp-gen-toggle-worldbook">在生成时使用所选条目</label>
              </div>
              <div id="sp-worldbook-list-container"></div>
              <div style="text-align: right; margin-top: 8px;">
                  <button id="sp-worldbook-refresh-btn">刷新列表</button>
              </div>
          `;
          const worldbookListContainer = document.getElementById('sp-worldbook-list-container');
          const worldbookToggleCheckbox = document.getElementById('sp-gen-toggle-worldbook');
          const refreshBtn = document.getElementById('sp-worldbook-refresh-btn');
          if (typeof window.sp_manuallySelectedEntries === 'undefined') {
              window.sp_manuallySelectedEntries = new Set();
          }
          const WORLDBOOK_TOGGLE_KEY = 'friendCircleUseWorldbook';
          worldbookToggleCheckbox.checked = localStorage.getItem(WORLDBOOK_TOGGLE_KEY) === 'true';
          worldbookToggleCheckbox.addEventListener('change', (event) => {
              localStorage.setItem(WORLDBOOK_TOGGLE_KEY, event.target.checked);
              debugLog(`使用世界书状态: ${event.target.checked ? '开启' : '关闭'}`);
          });
          async function loadWorldbooks() {
              worldbookListContainer.innerHTML = '正在加载世界书...';
              try {
                  const TavernHelper = window.TavernHelper;
                  if (!TavernHelper || typeof TavernHelper.getWorldInfoList !== 'function' || typeof TavernHelper.getLorebookEntries !== 'function') {
                      throw new Error('TavernHelper API 不可用或版本过旧。');
                  }
                  const worldbooks = await TavernHelper.getWorldInfoList();
                  if (!worldbooks || worldbooks.length === 0) { worldbookListContainer.innerHTML = '没有找到已加载的世界书文件。'; return; }
                  worldbookListContainer.innerHTML = '';
                  for (const book of worldbooks) {
                      const bookContainer = document.createElement('div'); bookContainer.className = 'sp-worldbook-book';
                      const bookHeader = document.createElement('div'); bookHeader.className = 'sp-worldbook-header'; bookHeader.textContent = `▼ ${book.name}`; bookContainer.appendChild(bookHeader);
                      const entriesContainer = document.createElement('div'); entriesContainer.className = 'sp-worldbook-entries'; entriesContainer.style.display = 'none'; bookContainer.appendChild(entriesContainer);
                      bookHeader.addEventListener('click', () => { const isVisible = entriesContainer.style.display === 'block'; entriesContainer.style.display = isVisible ? 'none' : 'block'; bookHeader.textContent = `${isVisible ? '▼' : '▲'} ${book.name}`; });
                      worldbookListContainer.appendChild(bookContainer);
                      const entries = await TavernHelper.getLorebookEntries(book.name);
                      if (entries && entries.length > 0) {
                          entries.forEach(entry => {
                              if (!entry.content?.trim()) return;
                              const entryLabel = document.createElement('label'); entryLabel.className = 'sp-worldbook-entry-label';
                              const checkbox = document.createElement('input'); checkbox.type = 'checkbox'; checkbox.dataset.entryContent = entry.content; checkbox.checked = window.sp_manuallySelectedEntries.has(entry.content);
                              checkbox.addEventListener('change', (event) => { const content = event.target.dataset.entryContent; if (event.target.checked) { window.sp_manuallySelectedEntries.add(content); } else { window.sp_manuallySelectedEntries.delete(content); } debugLog(`手动选择的世界书条目: ${window.sp_manuallySelectedEntries.size} 个`); });
                              entryLabel.appendChild(checkbox); entryLabel.appendChild(document.createTextNode(` ${entry.comment || entry.keys[0] || '无标题条目'}`));
                              entriesContainer.appendChild(entryLabel);
                          });
                      } else { entriesContainer.textContent = '（此世界书为空）'; }
                  }
              } catch (e) { worldbookListContainer.innerHTML = `加载失败: ${e.message}`; debugLog('加载世界书选择器失败', e); }
          }
          refreshBtn.addEventListener('click', loadWorldbooks);
          loadWorldbooks();
      }
      ...
      function showGenPanel() {
          content.innerHTML = `
              ...
              <div id="sp-gen-output" class="sp-output"></div>`;
          ...
          document.getElementById('sp-gen-now').addEventListener('click', async () => {
              try {
                  ...
                  if (localStorage.getItem(WORLDBOOK_TOGGLE_KEY) === 'true') {
                      if (window.sp_manuallySelectedEntries && window.sp_manuallySelectedEntries.size > 0) {
                          selectedWorldbooks = Array.from(window.sp_manuallySelectedEntries);
                          debugLog(`使用了 ${selectedWorldbooks.length} 个手动选择的世界书条目。`);
                      } else { debugLog('“使用世界书”已开启，但未在“世界书”面板中选择任何条目。'); }
                  } else { debugLog('已跳过读取世界书 (开关在“世界书”面板中关闭)'); }
                  await generateFriendCircle(selectedChat, selectedWorldbooks);
              } catch (e) {
                  console.error('生成异常:', e); debugLog('生成异常:', e.message || e);
                  if(outputContainer) { outputContainer.textContent = `发生错误: ${e.message}`; }
              }
          });
          async function generateFriendCircle(selectedChat = [], selectedWorldbooks = []) {
              ...
              const enabledPrompts = (JSON.parse(localStorage.getItem('friendCircleUserPrompts') || '[]')).filter(p => p.enabled).map(p => p.text), messages = [];
              ...
              debugLog('准备生成朋友圈，使用 API 信息:', { url, model }); debugLog('使用的提示词:', messages);
              try {
                  ...
                  outputContainer.textContent = output; localStorage.setItem(LAST_GEN_OUTPUT_KEY, output); debugLog('生成结果输出到面板:', output);
              } catch (e) {
                  console.error('生成朋友圈失败:', e); const errorMsg = '生成失败: ' + (e.message || e);
                  outputContainer.textContent = errorMsg; localStorage.setItem(LAST_GEN_OUTPUT_KEY, errorMsg); debugLog('生成失败', e.message || e);
              }
          }
          ...
          function toggleAutoMode(forceState) {
              autoMode = typeof forceState === 'boolean' ? forceState : !autoMode; localStorage.setItem(AUTO_MODE_KEY, autoMode ? '1' : '0');
              ...
              if (autoMode) {
                  ...
                  autoObserver = new MutationObserver(() => {
                      ...
                      if (ctx?.chat?.length > lastMessageCount) {
                          ...
                          if (newMsg && !newMsg.is_user && newMsg.mes) { debugLog('检测到新AI消息，触发自动生成'); (async () => { await generateFriendCircle(await getLastMessages(), []); })(); }
                      }
                  });
